...
def esa_scorer_lstm(y_val, y_pred, benchmark):
    pred_anomalies = benchmark.process_pred_anomalies(y_pred, 0)
...
def main():
    ...
    search_cv_factory3 = lambda: BayesSearchCV(
        ...
        scoring=make_scorer(partial(esa_scorer_lstm, benchmark=benchmark), greater_is_better=True),
        cv=TimeSeriesSplit(n_splits=3),  # standard CV con shuffle
        n_iter=30,        # meno iterazioni rispetto a modelli più veloci
    ...
    benchmark.run(
        ...
        search_cv_factory3=search_cv_factory3,
        perc_eval1=0.2,
        perc_eval2=0.2,
        perc_shapelet=0.1,
