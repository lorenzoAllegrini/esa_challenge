...
def main():
    ...
    differences_columns= [f"max_step_difference_{i}" for i in range(1,6)]
    differences_columns.extend([f"min_step_difference_{i}" for i in range(1,6)])
    selected_features = ["max_std", "min_slope", "max_slope", "max_var", "max_diff_var", "max_stft", "max_sc"]
    selected_features.extend(differences_columns)
    ...
    xgb_param_space = {
        "classifier__scale_pos_weight": Real(0.8, 1.0),
    ...
    search_cv_factory3 = lambda: BayesSearchCV(
        ...
        search_spaces=xgb_param_space3,
        scoring=xgb_scorer3,
