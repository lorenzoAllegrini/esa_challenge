...
def extract_kernels_from_data(X_train, window_size=50, n_kernels=100, method="per_segment", n_neighbors=20, random=False):
    ...
    if len(X_train) < window_size:
        ...
        if random:
            ...
            if max_start_idx < 1:
                raise ValueError("La serie temporale è troppo corta per estrarre finestre.")
            
            indices = np.random.choice(np.arange(max_start_idx), size=100_000, replace=False)
            segments = np.array([X_train[idx : idx + window_size] for idx in indices], dtype=np.float64)   
        else:
            segments = np.array(
                [X_train[i : i + window_size] for i in range(len(X_train) - window_size + 1)], dtype=np.float64
            )
...
class ShapeletMiner:
    ...
    """
    def __init__(self, anomaly_pools, nominal_segments, num_kernels=10, k_min_length=30, k_length=40, segment_duration=50, step_duration=1, n_jobs=-1):
        ...
        if not anomaly_pools:
            print("[RocketExtracted2] Nessun pool di anomalie: generazione kernel Dirichlet di fallback.")
            self.kernels = []
        ...
        self.kernels = self.select_best_kernels_by_clustering(all_kernels, all_scores, num_kernels)
    ...
    def _transform(self, X, y=None):
        ...
        raw_features = _apply_kernels2(X.astype(np.float32), self.process_kernels(self.kernels))
    ...
    def select_best_kernels_by_clustering(self, kernels, scores, num_kernels):
        if not scores:
            print("Nessuno score disponibile: restituisco lista vuota.")
            print(kernels)
            return []
        ...
        if len(groups) == 0:
            print("Nessun kernel ha superato il filtro iniziale. Uso tutti i kernel disponibili.")
            ...
            all_kernels = [(np.array(k).flatten(), score) for k, score in zip(kernels, scores)]
        ...
        sorted_lengths = sorted(unique_lengths, key=lambda l: group_avg_scores[l], reverse=True)
        
        # Debug: stampa allocazioni iniziali
        print("Allocazioni iniziali per gruppo:", allocations)
        ...
        for l in allocations:
            kernels_list = groups[l]           # lista dei kernel (flattened) del gruppo di lunghezza l
            ...
            if len(kernels_list) == k_clusters:
                ...
                kmeans = TimeSeriesKMeans(n_clusters=k_clusters, metric="euclidean", random_state=0).fit(data)
        ...
        print("Numero di centroidi estratti:", len(centroids))
    ...
    def score_anomaly_kernels(self, kernels_pool, test_pools):
        ...
        nominal_scores = self.compute_kernel_scores(pool=self.nominal_segments, kernels=kernels)
    ...
    def compute_kernel_scores(self, pool, kernels):
        ...
        if pool_array.ndim == 2:
            ...
            raise ValueError(f"Formato del pool non valido: shape {pool_array.shape}. Atteso (n, c, l).")
    ...
    def extract_kernels_from_data(self, X_train, length, method="per_segment"):
        ...
        if len(X_train) < length:
            ...
            if self.random:
                ...
                indices = np.random.choice(np.arange(max_start_idx), size=num_samples, replace=False)
                segments = np.array([X_train[idx : idx + length] for idx in indices], dtype=np.float64)   
            else:
                segments = np.array(
                    [X_train[i : i + length] for i in range(len(X_train) - length + 1)], dtype=np.float64
                )
