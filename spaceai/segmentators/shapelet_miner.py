...
class ShapeletMiner:
    ...
    def __init__(self, k_min_length:int, k_max_length:int, num_kernels:int, segment_duration:int, step_duration:int, n_jobs=-1, 
                 run_id: str = "esa_segments", exp_dir: str = "experiments", skip=True):
        self.run_id = run_id
    ...
    def initialize_kernels(self, esa_channel:ESA, mask:Tuple[int,int], ensemble_id: str):
        channel_dir = os.path.join(
            self.exp_dir, self.run_id,
            "channel_segments", esa_channel.channel_id
        )
        ...
        if os.path.exists(shapelets_path) and os.path.getsize(shapelets_path) > 0:
            try:
                ...
                print(f"[ShapeletMiner] WARN: {shapelets_path} malformato, rigenero tutto.")
        ...
        for idx, (s, e) in enumerate(anomalies):
            ...
            padded_intervals.append((s - 2 * self.segment_duration, e + 2 * self.segment_duration))
        ...
        while i + self.segment_duration < mask[1]:
            if not any(max(i, a) < min(i + self.segment_duration, b) for a, b in merged):
                nominal_segments.append(data[i:i + self.segment_duration, 0])  
                i += self.segment_duration
            else:
                j = next(b for a, b in merged if max(i, a) < min(i + self.segment_duration, b))
                i = j + 1
        ...
        for i, pool in enumerate(anomaly_pools):
            ...
            kernels, scores = self.score_anomaly_kernels(pool, other_pools, nominal_segments)
    ...
    def score_anomaly_kernels(self, kernels_pool, test_pools, nominal_segments):
        ...
        nominal_scores = self.compute_kernel_scores(pool=nominal_segments, kernels=kernels)
